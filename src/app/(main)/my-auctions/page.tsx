    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      {leiloes?.length === 0 ? (
        ...
        <div className="grid gap-1">
          {leiloes?.map((leilao) => (
            <Link key={leilao.id} href={RoutesScreenPaths.AUCTION_DETAIL(leilao.id)} className="block mb-4 cursor-pointer group">
              <div key={leilao.id} className="bg-white border rounded-xl p-6 shadow-sm flex items-center justify-between cursor-pointer">
                <div className="relative h-24 w-40 flex-shrink-0 overflow-hidden rounded-lg border bg-gray-100">
                  {leilao.imageUrl && leilao.imageUrl.length > 0 ? (
                    <img
                      src={leilao.imageUrl} // Ou o caminho que vier do seu back-end
                      alt={leilao.title}
                      className="h-full w-full object-cover"
                    />
                  ) : (
                    /* Ícone de fallback caso não haja foto */
                ...
                <div >
                  ...
                  <div className="flex gap-6 mt-5 text-sm text-gray-600">
                    ...
                    <Badge className='bg-gray-200 text-black'>Lance atual: R$ {leilao.currentPrice.toLocaleString('pt-BR')}</Badge>
                    <Badge className='bg-gray-200 text-black'>Quantidade de lances: {leilao.bidCount.toLocaleString('pt-BR')}</Badge>
                    <Badge className={`
                        ${leilao.actualWinner ? 'bg-green-800 text-white' : 'bg-gray-200 text-black'} 
                        transition-colors duration-200
                      `}>Maior lance: {leilao.actualWinner}</Badge>
                    <Badge className='bg-gray-200 text-black'>Termina em: {new Date(leilao.endDate).toLocaleDateString('pt-BR')}</Badge>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 mt-10">
                  ...
                  <ButtonCustom
                    className="hover:underline font-medium rounded-xl cursor-pointer"
                    onClick={(e: any) => {
                      e.preventDefault(); 
                      e.stopPropagation(); 
